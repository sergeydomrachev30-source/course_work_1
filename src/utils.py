...
def process_card_data(card_info: pd.DataFrame, cashback_rate: float = 0.01) -> tuple[str | None, float, float]:
    """
    ...
    logging.info(f"Обработаны данные карты: {last_four}, траты {total_spent}, кешбэк {cashback}.")
...
def get_sp500_stocks() -> list[dict[str, Any]]:
    """
    ...
    if response.status_code == 200:
        ...
        raise Exception("API запрос не удался")
...
def calculate_expenses_transfers(df: pd.DataFrame, start_date: str, end_date: str) -> dict:
    ...
    transfers_and_cash_summary = (
        transfers_and_cash.groupby("Категория")["Сумма операции"].sum().sort_values(ascending=False)
    )
    transfers_and_cash_data = [
        {"category": cat, "amount": abs(int(amount))} for cat, amount in transfers_and_cash_summary.items()
    ]

    logging.info("Завершение расчета расходов и переводов")
    return {
        "expenses": {"total_amount": total_expenses, "main": main_categories},
...
def calculate_incomes(df: pd.DataFrame, start_date: str, end_date: str) -> dict:
    ...
    return {"total_amount": total_income, "main": main_income_categories}
