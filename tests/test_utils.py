...
def card_data_fixture():
    return pd.DataFrame({
        ...
        'Сумма операции': [-1000, -2000]  # Отрицательные значения для трат
    })
...
def mock_response_data():
    return {
        "securities": {
            "data": [
                ["ABRD", "TQBR", "АбрауДюрсо", 158.2],
                ["AFLT", "TQBR", "Аэрофлот", 51.8]
            ]
        }
    }
...
@pytest.mark.parametrize("start_date,end_date,expected_total", [
...
def test_calculate_expenses_transfers(transaction_data_fixture, start_date, end_date, expected_total):
    ...
    transaction_data_fixture['Дата платежа'] = pd.to_datetime(transaction_data_fixture['Дата платежа'], format='%d.%m.%Y')
...
@pytest.mark.parametrize("start_date,end_date,expected_total", [
    ('2023-01-01', '2023-01-31', 1700),
])
def test_calculate_expenses_transfers(expense_data_fixture, start_date, end_date, expected_total):
    result = calculate_expenses_transfers(expense_data_fixture, start_date, end_date)
    assert result['expenses']['total_amount'] == expected_total
...
@pytest.fixture
def income_data_fixture():
    return pd.DataFrame({
        'Дата платежа': ['2023-01-01', '2023-01-15', '2023-01-20'],
        'Категория': ['Зарплата', 'Бонус', 'Подарки'],
        'Сумма операции': [3000, 500, 200]
    })

@pytest.mark.parametrize("start_date,end_date,expected_total", [
    ('2023-01-01', '2023-01-31', 3700),
])
def test_calculate_incomes(income_data_fixture, start_date, end_date, expected_total):
    result = calculate_incomes(income_data_fixture, start_date, end_date)
    assert result['total_amount'] == expected_total
